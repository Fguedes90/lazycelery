...
impl RedisBroker {
                ...
                if let Ok(task_data) = serde_json::from_str::<Value>(&data) {
                    let task = Task {
                        id: key
                            ...
                            .unwrap_or("unknown")
                            .to_string(),
                        name: task_data["task"].as_str().unwrap_or("unknown").to_string(),
                        args: task_data["args"].to_string(),
                        kwargs: task_data["kwargs"].to_string(),
                        ...
                        worker: task_data["hostname"].as_str().map(|s| s.to_string()),
                        timestamp: Utc::now(), // Would parse from task data
                        result: task_data["result"].as_str().map(|s| s.to_string()),
                        traceback: task_data["traceback"].as_str().map(|s| s.to_string()),
                    };
                    tasks.push(task);
                }
            }
    ...
    async fn parse_queues(&self) -> Result<Vec<Queue>, BrokerError> {
        ...
        for name in queue_names {
            let length: u64 = conn.llen(name).await.unwrap_or(0);
